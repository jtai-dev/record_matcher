...
class TBConfigXColumnNotFound(Exception):
    def __init__(self, column, x_columns:list):
        super().__init__(f"x_column:{column} cannot be found in x_records. Only these " \
                         f"x_columns may be used: {', '.join(x_columns)}")


class TBConfigYColumnNotFound(Exception):
    def __init__(self, column, y_columns:list):
        super().__init__(f"y_column:{column} cannot be found in y_records. Only these " \
...
class TBConfigXUniqueConstraint(Exception):
    def __init__(self, column, config_classname) -> None:
        super().__init__(f"x_column:{column} already exists in {config_classname} values.")


class TBConfigOverwriteError(Exception):
    def __init__(self, column):
        super().__init__(f"x_column:{column} is a column in x_records. "\
...
class TBConfigScorerNotFound(Exception):
    def __init__(self, scorer, scorers):
        super().__init__(f"Scorer name: \'{scorer}\' is not found in scorers. " \
...
class TBConfigColumnToMatchLock(Exception):
    def __init__(self, column):
        super().__init__(f"x_column:{column} is a column to be matched. Please remove it " \
